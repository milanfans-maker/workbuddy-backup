...
description_zh: 零依赖提取PDF与图片文字，扫描件交AI视觉识别，支持表格结构化与批量处理
description_en: Extract text from PDFs and images with zero extra dependencies; scanned pages use AI vision, plus table and batch support.
...
前往 [红狐hub](https://redfox.hk/settings/api-keys?source=workbuddy) 注册获取个人 API Key，通过以下任一方式配置：
...
> 「需要配置红狐 API Key 才能使用本 Skill。请前往 https://redfox.hk/settings/api-keys?source=workbuddy **免费**注册获取，然后配置环境变量：`export REDFOX_API_KEY="ak_xxxx..."`」
...
- 脚本以非零退出码终止 → **立即停止**，向用户展示脚本输出的错误提示，告知需前往 [红狐hub](https://redfox.hk/settings/api-keys?source=workbuddy) 免费获取 API Key
...
2. **立即停止**，向用户输出：「需要配置红狐 API Key 才能使用本 Skill。请前往 https://redfox.hk/settings/api-keys?source=workbuddy **免费**注册获取，然后执行：`export REDFOX_API_KEY="ak_xxxx..."`」
